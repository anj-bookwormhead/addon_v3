...
document.addEventListener('change', function (e) {
    ...
    if (input.matches('input[type="checkbox"][data-addon-key]')) {
        ...
        document.querySelectorAll('input[type="checkbox"][data-addon-key]:checked').forEach((checkbox) => {
        ...
        jQuery.post(wc_ajax_data.ajax_url, {
            ...
            console.log('✅ Session Updated:', response);
            // 🔄 Refresh checkout to recalculate fees
            jQuery('body').trigger('update_checkout');
        });
    }
});
...
document.addEventListener('change', function (e) {
    ...
    if (input.matches('input[type="checkbox"][data-addon-key]')) {
        ...
        // ✅ Always rebuild full list from scratch (not relying on old array)
        document.querySelectorAll('input[type="checkbox"][data-addon-key]:checked').forEach((checkbox) => {
            const key = checkbox.dataset.addonKey;
            const addon = participant_addons_data.addons.find(a => a.field_name === key);
            if (addon) {
        ...
        // ✅ Always trigger AJAX — even if selected is empty
        jQuery.post(wc_ajax_data.ajax_url, {
            action: 'update_custom_addons',
            addons: selected
        }, function (response) {
